...
vs100sp1_is_installed = is_vs100sp1_installed?()

install_url = File.join(node['visualstudio']['source'], node['visualstudio']['100']['sp1']['filename'])
...
setup_exe_path = File.join(iso_extraction_dir, node['visualstudio']['100']['sp1']['installer_file'])

seven_zip_archive 'extract_iso' do
  path iso_extraction_dir
  source install_url
  overwrite true
  checksum node['visualstudio']['100']['sp1']['checksum']
  not_if { vs100sp1_is_installed }
end

windows_package node['visualstudio']['100']['sp1']['package_name'] do
